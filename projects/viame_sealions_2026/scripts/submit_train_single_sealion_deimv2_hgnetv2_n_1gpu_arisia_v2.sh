...
#   version:  v2
#
# Sibling of submit_train_pup_vs_nonpup_deimv2_hgnetv2_n_1gpu_arisia_v4.sh;
# only the scheme + category_names differ. All other hyperparameters
# match so cross-scheme comparisons are meaningful.
#
# Changes vs v1:
#   - Batch per-GPU: 128 -> 48. v1's batch=128 was inherited from the
#     pup_vs_nonpup v2 OOM config; replaced here with the v4 settings
#     that ran stably (33-44 GB on the A6000).
#   - LR: 1.6e-3 -> 9.8e-4 (sqrt(48/32) * v1-pup's 8e-4).
#   - Backbone LR: 8e-4 -> 4.9e-4 (same scaling).
#
...
#   bash projects/viame_sealions_2026/scripts/submit_train_single_sealion_deimv2_hgnetv2_n_1gpu_arisia_v2.sh
set -euo pipefail
